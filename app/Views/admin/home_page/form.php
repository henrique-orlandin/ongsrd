...
<section class="panel form-panel">
    <h2><i class="fa-solid fa-house"></i>Configurações da Página Inicial</h2>
    <form class="dropzone upload-image" method="post" enctype="multipart/form-data" action="<?= base_url('cms/home-page/update') ?>">
        <?= csrf_field() ?>
        <label> Descrição <textarea name="description" rows="7" required><?= esc(old('description', $item['description'] ?? '')) ?></textarea></label>
        <label>Imagem (opcional)
            <div id="home-dropzone" class="upload-dropzone my-dropzone">
...
<script>
    ...
    const dropzoneSelector = '#home-dropzone';
    ...
    new Dropzone(dropzoneSelector, {
        ...
        init: function() {
            ...
            this.on('success', function(file, response) {
                if (response.success) {
                    window.location.href = "<?= base_url('cms/home-page') ?>";
                    return;
                }
